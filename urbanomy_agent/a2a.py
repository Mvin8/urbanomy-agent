...
def build_card(settings):
    ...
    properties = {
        ...
        "scenario_id": {"type": "string", "description": "Prepared scenario directory under DATA_DIR. Use list_scenarios; never infer an id."},
        "project_id": {"type": "string", "description": "Optional project label, saved in provenance; does not select data."},
        "target_id": {"type": "string", "description": "Exact block id from the dataset, as a string. Never invent an id."},
        "constraints_json": {"type": "string", "description": 'For optimization: JSON object of explicit bounds, e.g. {"l":{"min":1,"max":8}}. Do not invent limits. Overrides profile bounds per parameter; without a profile omitted variables stay fixed.'},
        "constraints_profile": {"type": "string", "enum": ["test"], "description": "Optional test preset: footprint 1 m² to 10% of site area, floors 1–10, MXI 0.1–1, all shares 0–1. Explicit bounds override it."},
    ...
    card = {
        ...
        "capabilities": {"streaming": True, "extensions": [{
            ...
            "description": "Structured Urbanomy input; constraints_json supports scalar parameter extraction.",
            "params": {"type": "object", "properties": properties,
                       "required": ["operation", "scenario_id", "target_id"], "additionalProperties": False},
        }]},
