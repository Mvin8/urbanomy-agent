...
├── prompts/                    # промпты на английском для внешних агентов
│   ├── README.md               # подключение промптов и обязанности среды
│   ├── urbanomy-specialist.md  # системные инструкции MCP-специалиста
│   ├── urbanomy-routing.md     # маршрутизация экономических задач
│   └── optimizer-strategy.txt  # эталонный критерий оценки стратегии
├── examples/
│   ├── main.ipynb              # исследовательский пример
│   └── integration/            # MCP/A2A-запросы, ответы, GeoJSON и метаданные прогонов
│       ├── README.md           # состав примеров и повторный запуск
│       ├── agent-evals.md      # сценарии проверки поведения агентов
│       └── benchmark.json     # результаты локальных замеров ресурсов
├── scripts/
│   ├── generate_docs.py        # генерация каталога MCP, схем и Agent Card
│   ├── smoke_server.py         # проверка MCP/A2A на работающем сервере
│   ├── replay_integration.py   # повторение эталонных запросов через HTTP MCP
│   └── benchmark_integration.py # локальные замеры времени, CPU и памяти
├── tests/                      # проверки расчётов, контрактов и транспорта
└── docs/                       # архитектура, данные, контракты, развёртывание
    └── integration.md          # демо, эталонные результаты, ресурсы и интеграция
```
...
Подготовлены эталонные запросы и результаты расчётов без LLM, локальные замеры
ресурсов и промпты для подключения внешних агентов. Сценарии проверки поведения
агентов пока не прогнаны на целевой модели в Synapse.
