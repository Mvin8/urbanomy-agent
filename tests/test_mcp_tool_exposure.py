...
def test_mcp_catalog_and_invocation(settings):
    ...
    async def check():
        ...
        assert {"dataset_id", "target_id", "constraints", "strategy"} <= set(tool.inputSchema["required"])
        result = await mcp.call_tool("estimate_land_value", {"dataset_id": "test", "target_id": 86})
...
def test_mcp_http_requests_do_not_close_job_manager(settings):
    ...
    with TestClient(build_app(settings, service)) as client:
        ...
        result = call("tools/call", {"name": "estimate_land_value", "arguments": {"dataset_id": "test", "target_id": 86}})
