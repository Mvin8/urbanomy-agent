...
`urbanomy` содержит расчётную библиотеку, `urbanomy_agent` — A2A-адаптер и сервис
выполнения расчётов, `urbanomy_mcp` — инструменты MCP.
Библиотека не импортирует серверные пакеты; сервис использует её через `engine.py`.
...
| `urbanomy/land_value/` | Модель стоимости, сценарии и исходный оптимизатор |
| `urbanomy/investment/` | NPV и экономические предположения |
